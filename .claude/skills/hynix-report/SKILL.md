...
## 8. 투자 판단                        ← **매수/중립/매도** + 4대 요소(추세·밸류에이션·촉매·산업사이클) 방향 표 + 판단을 바꿀 조건
...
## 투자 판단 기준 (방향성 기반, 2026-09-08 개정)

투자의견(매수/중립/매도)은 **절대 가격 레벨이 아니라 향후 방향성**으로 판단한다 — "지금 가격이 어디에 있는지"가 아니라 "앞으로 오를 가능성이 높은지 내릴 가능성이 높은지"가 근거다. 판정 변경은 **오직 종가(15:30 확정) 기준**으로만 이루어지며, 장중 등락으로는 절대 바꾸지 않는다. 모든 리포트에 "판정은 종가 기준으로만 변경합니다"를 명시한다.

(참고: 2026-07-21경 도입돼 쓰이던 고정 절대가격선 체계(매도 152만/매수 173.5만 등)는 최초 설정 후 재검토 로직이 없어 주가가 그 뒤 20%+ 올라도 그대로 방치되는 구조적 결함이 있었다. 2026-09-08에 완전 폐기하고 아래 방향성 기반 체계로 전환.)

**4대 판단 요소** (매 리포트에서 이미 조사하는 항목으로 그대로 판단 — 별도 신규 조사 불필요):

| 요소 | 판단 근거 | 방향 |
|---|---|---|
| ① 추세/모멘텀 | MACD(골든/데드크로스, 0선 상하), ADX/DMI(+DI vs -DI 우위), KDJ, 20일 이평 대비 위치 — 최근 2~3일 흐름을 종합(1일 스냅샷만으로 판단 금지) | 상승 / 중립 / 하락 |
| ② 밸류에이션 갭 | 컨센서스 목표주가 대비 현재가 괴리율 = (목표주가−현재가)/현재가 | 상승여력(할인) / 중립 / 과열(프리미엄) |
| ③ 촉매 | 최신 뉴스 Top5 중 호재(🟢):악재(🔴) 비중, 임박한 실적·이벤트(D-day)의 예상 영향 | 우호적 / 중립 / 비우호적 |
| ④ 산업 사이클 | DRAM/NAND/HBM 가격·수급 전망(공급부족·과잉), HBM 점유율 추이 | 개선 / 중립 / 둔화 |

**종합 판정 규칙:**
- **매수**: 4개 요소 중 3개 이상 "긍정"이고, "부정"은 0개
- **매도**: 4개 요소 중 3개 이상 "부정"이고, "긍정"은 0개
- **중립**: 그 외 모든 경우 (신호 혼재·애매)

**판정 변경(휘핑소 방지) 규칙:**
- 위 종합 판정 조건을 종가 기준으로 충족하지 못하면(신호가 애매하면) 기존 판정을 그대로 유지한다 — 조건 미충족 시 "전환 안 함"이 기본값이다.
- 요소별 방향은 해당 지표군의 최근 2~3일 추세로 판단하며, 단일 지표의 하루짜리 노이즈(예: MACD 하루 반전)만으로 그 요소의 방향을 뒤집지 않는다.
- 매 리포트 "투자 판단" 절에 4대 요소 각각의 현재 방향과 근거를 표로 명시해, 판정이 어떤 조합에서 나왔는지 투명하게 보여준다.
- 실적 서프라이즈·대형 계약·제재 등 급변 뉴스로 여러 요소가 한꺼번에 급반전하면 중립을 거치지 않고 매수↔매도 직행 전환도 가능하다 — 이는 정상 동작이며 막을 대상이 아니다.
