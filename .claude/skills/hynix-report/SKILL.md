...
## 투자 판단 임계값 (매수/중립/매도 전환 규칙)

투자의견은 **오직 종가(15:30 확정) 기준**으로만 판정을 변경한다. 장중 등락으로는 절대 바꾸지 않으며, 모든 리포트에 "판정은 종가 기준으로만 변경합니다"를 명시한다.

2026-09-08 기준 현재 임계값 (가격이 크게 재평가되면 재검토 필요 — 고정불변 아님):

| 선 | 가격 | 역할 |
|---|---|---|
| 매도 재전환선 | 152.0만 (1,520,000원) | 이 아래로 종가 마감 시 매도 진입 |
| 매도 이탈선 | 157.0만 (1,570,000원) | 매도 상태에서 이 이상 회복 시 중립 복귀 |
| 매수 이탈선 | 168.0만 (1,680,000원) | 매수 상태에서 이 아래로 하회 시 중립 복귀 |
| 매수 복귀선 | 173.5만 (1,735,000원) | 이 이상으로 종가 마감 시 매수 진입 |

**상태별 분기 (당일 종가 P로 판정, 현재 상태에 따라 다름):**
- **매도** 상태: P ≥ 173.5만 → 매수(직행) · P ≥ 157만 → 중립 · 그 외 → 매도 유지
- **중립** 상태: P ≥ 173.5만 → 매수 · P < 152만 → 매도 · 그 외 → 중립 유지
- **매수** 상태: P < 152만 → 매도(직행) · P < 168만 → 중립 · 그 외 → 매수 유지

이탈선(157만/168만)은 진입선(152만/173.5만) 대비 각각 약 +3.3%/-3.2% 완충폭이다. 진입선끼리는 그대로 유지한 채, 각 극단 상태(매도·매수) 안에서 이탈만 별도 임계값으로 분리해 종가가 진입선 부근에서 며칠씩 오르내릴 때 판정이 매일 뒤집히는 whipsaw를 막기 위해 2026-09-08에 도입했다(중립 상태 자체의 폭은 기존 152만~173.5만 그대로 유지). 급락·급등으로 한 번에 반대 극단까지 종가가 도달하면(예: 매도 상태에서 종가가 바로 173.5만 이상으로 마감) 중립을 거치지 않고 직행 전환한다 — 이 자체는 정상 동작이며 막을 대상이 아니다.
